...
function executeCommand () {
	...
	if (command.length == 0 || command == "") {
		terminalOut("<p><strong>ERROR!!:</strong> Escribe lo que queires hacer</p>");
		return;
	}
	if (command.length == 1) {
...
function parseCommand (command) {
	...
	case 'ir':
		...
		terminalOut("<p>Esta puerta esta disponible: " + doors + "</p>");	
		break;
	...
	case 'coger':
		...
		terminalOut("<p>Los items disponibles son: " + items + "</p>");
		break;
	...
	case 'inventario':
			...
			terminalOut("<p>Lo que tienes en el inventario es: " + items_inventory + "</p>");
			break;	
		default:
			terminalOut("<p><strong>ERROR!!:</strong> Este comando <strong>" + command + "</strong> no se sabe cual es</p>");
	}
}
...
function parseInstruction (instruction) {
	...
	case 'ver':
		...
		if (item_number < 0) {
			terminalOut("<p>El objeto<strong> " + instruction[1] + "</strong> no esta en esta habitación</p>");
			return;
		}	
	...
	case 'ir':
		...
		if (door_number < 0) {
			console.log("Error no es esta puerta");
			return;
		}	
		...
		game_data.rooms[current_room].items.forEach(function (item) {
			if (item == instruction[1]) {				
				...
				if (game_data.items[item_num].pickable == false) {
					terminalOut("<p>El objeto<strong> " + item + "</strong> no se puede coger</p>");
					return;
				}					
				...
				terminalOut("<p>El objeto<strong> " + item + "</strong> ahora esta en tu inventario</p>");
				return;
			}
		});
		break;
	...
	case 'inventario':
		...
		default:
			terminalOut("<p><strong>ERROR:</strong> Este Comando <strong>" + instruction[0] + "</strong> no se sabe cual es</p>");
	}
}
